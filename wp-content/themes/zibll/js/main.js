 ...
 * @LastEditTime: 2022-02-28 22:26:15
...
function post_ajax(_this, con, jcon, item, loader, pag, next, trigger, replace, nomore, top, ajax_trigger) {
    ...
    href && $.ajax({
        ...
        success: function (a) {
            ...
            $con.append(loader).removeAttr('ing').trigger('ajax.ed').find(">.post_ajax_loader").hide();
...
if (_wid < 768 && $('.drawer-sm').length) {
    _win.bd.on('click', '[data-drawer]', function (e) {
        ...
        $(drawer_selector).Drawer(action, title);
    });
}

$.fn.Drawer = function (action, title) {
    title = title || '';
    var _this = $(this);
    if (!_this.length) return;
...
//内容浮动
$.fn.autoAffix = function (top = 0, bottom = 0) {
    var _this = $(this);
    if (!_this.length) return;
    var _body = $('body');
    var nav_Height = _body.hasClass("nav-fixed") ? $(".header").outerHeight() : 0;
    ...
    function get_offset() {
        return {
            top: _this.parent().offset().top - nav_Height + offset_top,
...
_win.bd.on('click', '[data-for]', function () {
    ...
    if (multiple) {
        ...
        _this.parent().find('.active').each(function (indexInArray, valueOfElement) {
            var _this_2 = $(this);
            _tt += _this_2.html();
